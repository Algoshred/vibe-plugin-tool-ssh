...
interface CreatePortForwardBody {
  localPort: number;
  remoteHost: string;
  remotePort: number;
  connectionId: string;
...
export const portForwardRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/", async (request, reply) => {
    const { localPort, remoteHost, remotePort, connectionId } =
      request.body as CreatePortForwardBody;

    try {
      const existing = fastify.db
        .getAllPortForwards()
        .find(
          (pf: { localPort: number; status: string }) =>
            pf.localPort === localPort && pf.status === "active",
        );
